...
class Sentiment {
	...
	public function score($sentence) {
		...
		foreach ($this->classes as $class) {
			...
			foreach ($tokens as $token) {
				//echo $this->dictionary[htmlentities("不好")]["neg"];
	...
	public function setDictionary($class) {
		 ...
		 */
		$fn = "{$this->dataFolder}source.{$class}.php";
	...
	public function setDataFolder($dataFolder = false, $loadDefaults = false){
		...
		if($dataFolder == false){
			$this->dataFolder = __DIR__ . '/dictionaries/';
		}
		else{
			if(file_exists($data)){
	...
	public function getList($type) {
		...
		$fn = "{$this->dataFolder}source.{$type}.php";
		;
		if (file_exists($fn)) {
			include($fn);
